...
instance GetProgram (Quote (Term TyName Name ())) where
    getProgram =
        asIfThrown .
        fmap (trivialProgram . void) . compileAndMaybeTypecheck True
    ...
    -- names during compilation that are not fresh
     do
        ...
        -- need our own typechecker pipeline to allow normalized types
            PLC.typecheckTerm
                (PLC.TypeConfig True mempty mempty mempty Nothing)
                compiled
        pure compiled
...
serialization =
    testNested
        ...
        , goldenPir
              ...
              (roundTripPirTerm $ runQuote maybePir)
